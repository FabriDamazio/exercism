...
pub fn anagrams_for<'a>(word: &str, possible_anagrams: &[&'a str]) -> HashSet<&'a str> {
    ...
    Para cada palavra candidata:
        ...
        Ignore se for idêntica à original (mesmo considerando case-insensitivity).

        Normalize (lowercase).

        Compare a versão ordenada da candidata com a da original.

        Se bater, é um anagrama → adicione ao HashSet.
    */
